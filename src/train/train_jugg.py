...
# ──────────────────────────────────────────────────────────────────────────────
# ★ NEW: Merge aesthetic model (e.g. Juggernaut-XL-v9) into SDXL Inpainting UNet
# ──────────────────────────────────────────────────────────────────────────────

def merge_unet_from_aesthetic_model(
    unet_inpaint: UNet2DConditionModel,
    aesthetic_model_path: str,
    alpha: float = 0.3,
    weight_dtype: torch.dtype = torch.float32,
) -> None:
    """
    Selective in-place merge: only merge TRANSFORMER (attention/ff) layers from
    the aesthetic model. Keep ALL ResNet, conv, time-embedding, and conditioning
    layers from the inpainting model to preserve mask/masked-image conditioning.

    Merged layers (contain '.attentions.' in key name):
      - Self-attention (attn1): to_k, to_q, to_v, to_out
      - Cross-attention (attn2): to_k, to_q, to_v, to_out
      - Feed-forward (ff): net.0.proj, net.2
      - Transformer norms & projections: norm1, norm2, norm3, proj_in, proj_out

    Kept from inpainting (everything else):
      - conv_in (9 channels), conv_out
      - All ResNet blocks (resnets.*.conv1/conv2/time_emb_proj, norm1/norm2)
      - Downsamplers, upsamplers
      - time_embedding, add_embedding
      - conv_norm_out

    Args:
        unet_inpaint: Already-loaded SDXL Inpainting UNet (9 in_channels).
        aesthetic_model_path: HuggingFace ID or local path of aesthetic SDXL model
                              (e.g. "RunDiffusion/Juggernaut-XL-v9").
        alpha: Weight for aesthetic model (0.3 = 30% aesthetic + 70% inpainting).
               Only applied to transformer/attention layers.
        weight_dtype: dtype to load the aesthetic UNet.
    """
    logger.info(
        f"Selective aesthetic merge from '{aesthetic_model_path}' "
        f"(α={alpha:.0%} aesthetic, attention-only)..."
    )

    # Load aesthetic UNet (4 in_channels) temporarily
    # Try variant="fp16" first (e.g. Juggernaut-XL-v9 only has fp16 weights),
    # then fall back to default variant.
    try:
        unet_aesthetic = UNet2DConditionModel.from_pretrained(
            aesthetic_model_path, subfolder="unet",
            low_cpu_mem_usage=True, torch_dtype=weight_dtype,
            variant="fp16",
        )
    except (OSError, EnvironmentError):
        logger.info("fp16 variant not found, trying default variant...")
        unet_aesthetic = UNet2DConditionModel.from_pretrained(
            aesthetic_model_path, subfolder="unet",
            low_cpu_mem_usage=True, torch_dtype=weight_dtype,
        )
    sd_aesthetic = unet_aesthetic.state_dict()
    sd_inpaint = unet_inpaint.state_dict()

    merged_count = 0
    kept_inpaint = 0

    for key in sd_inpaint.keys():
        # Keep conv_in from inpainting (9 input channels for mask+masked_image)
        if key.startswith("conv_in."):
            kept_inpaint += 1
            continue

        # ★ SELECTIVE MERGE: Only merge transformer/attention layers.
        # These contain aesthetic/style information (lighting, color, texture).
        # ResNet/conv/conditioning layers are critical for inpainting and must
        # be kept from the inpainting model.
        is_transformer_layer = ".attentions." in key

        if is_transformer_layer and key in sd_aesthetic and sd_inpaint[key].shape == sd_aesthetic[key].shape:
            sd_inpaint[key] = alpha * sd_aesthetic[key] + (1.0 - alpha) * sd_inpaint[key]
            merged_count += 1
        else:
            kept_inpaint += 1

    unet_inpaint.load_state_dict(sd_inpaint)

    # Free aesthetic UNet
    del unet_aesthetic, sd_aesthetic, sd_inpaint
    gc.collect()
    if torch.cuda.is_available():
        torch.cuda.empty_cache()

    logger.info(
        f"UNet selective merge done: {merged_count} attention tensors merged (α={alpha}), "
        f"{kept_inpaint} tensors kept from inpainting (ResNet/conv/conditioning)."
    )
...
def log_validation(
    ...
    for idx, img_path in enumerate(tqdm(val_subset, desc="Validation", leave=False)):
        ...
        # ── Binarize dilated mask (pipeline needs clean 0/255) ─────────────
        mask_for_pipe = pil_mask_dilated.point(lambda x: 255 if x > 127 else 0)

        # ── Inpaint with BINARY mask, DEPTH, and quality boost ───────────
        with autocast_ctx:
            out = pipeline(
                prompt=prompt,
                image=pil_orig,
                mask_image=mask_for_pipe,       # ★ binary mask (NOT blurred)
        ...
        # ★ Feather blend — blur mask AFTER inference for smooth compositing
        out = feather_blend(
            original=pil_orig,
            inpainted=out,
            mask=pil_mask_dilated,
            feather_radius=int(res * 0.015),  # ~15px at 1024 resolution
        )
...
def main(cfg):
    ...
    for i in range(num_local):
        if i == local_rank:
            ...
            # ★ Merge aesthetic model (e.g. Juggernaut-XL-v9) if configured
            aesthetic_model = getattr(cfg.model, "aesthetic_model_path", None)
            if aesthetic_model:
                merge_alpha = float(getattr(cfg.model, "aesthetic_merge_alpha", 0.7))
                merge_unet_from_aesthetic_model(
                    unet, aesthetic_model, alpha=merge_alpha, weight_dtype=weight_dtype
                )
            ...
            if accelerator.sync_gradients:
                ...
                if global_step % cfg.validation.validation_steps == 0:
                    ...
                    if accelerator.is_main_process:          # chỉ rank 0 chạy
                        ...
                        inference_scheduler = EulerDiscreteScheduler.from_config(
